...
Aqui você encontrará detalhes sobre a solução ideal para sua instituição se integrar no ecossistema do Open Finance Brasil. O produto da **Opus Software** é responsável por garantir a boa operação da sua Instituição com as demandas exigidas pelos orgãos reguladores, além de garantir a atualização de novas versões e evoluções de funcionalidades definidas pelas normas regulatórias.

A plataforma foi projetada para incorporar toda a complexidade regulatória do Open Finance Brasil, a camada que não traz o diferencial competitivo para sua Instituição. Dessa forma, você pode focar em **seus objetivos estratégicos** enqaunto cuidamos da parte regulatória para você.
...
**COMPLEMENTAR O GUIA DE LEITURA PARA DETENTOR E TRANSMISSOR**
...
- [O que é o perfil ITP e como ele se encaixa no Open Finance](/docs/itp): Visão geral do papel do ITP e meios de pagamento.
- [O ecossistema do Open Finance Brasil](/docs/ecossistema): Como o ecossistema funciona e quais são os perfis participantes.
- [Jornada de Consentimento para Pagamentos](/docs/jornadaconsentimento): Como o usuário autoriza um pagamento e como essa jornada se conecta ao papel do ITP.
...
- [Iniciação de Pagamento PIX](/docs/iniciacaodepagamento): Consentimentos para pagamentos atômicos ou agendados:
  - Pagamento imediato;
  - Pagamento agendado.
- [Pagamentos Automáticos](/docs/pagamentoautomatico): Consentimentos recorrentes para débitos periódicos (assinaturas, mensalidades, parcelas):
  - PIX automático;
  - Transferências inteligentes.
- [Vínculo de dispositivo](/docs/vinculodedispositivo): Permissão de vínculo de dispositivo para a realização de jornadas simplificadas:
  - JSR - Jornada sem redirecionamento: Modalidade de pagamento em que o usuário autoriza a transação sem ser redirecionado para o ambiente da Detentora de Conta;
  - PIX por aproximação: Modalidade de pagamento em que a transação Pix é iniciada via NFC, permitindo que o usuário efetue pagamentos aproximando o celular de uma maquininha ou terminal, sem precisar abrir o aplicativo ou escanear um QR Code.
- [Cenários de pagamento](/docs/cenariospagamentos): Casos de uso práticos e combinações de tipos de pagamento.

#### 5. Fluxos técnicos e redirecionamento

- [Funcionamento geral da Iniciação de Pagamentos e Recepção de Dados](/docs/funcionamento): Visão dos fluxos de negócio: listagem de participantes, criação de consentimento, redirecionamento e consulta de status;
- [Redirecionamento App-to-App e Web](/docs/redirecionamento): Como o usuário deve ser redirecionado à Detentora de Conta para autorizar o pagamento e como deve retornar ao ambiente do ITP;
- [Webhooks de pagamentos](/docs/webhooks): Como receber notificações assíncronas sobre mudanças de status dos pagamentos.
...
- [O que é o perfil Receptor de Dados](/docs/receptordedados): Visão geral do papel do Receptor, os dados que podem ser compartilhados (cadastrais, conta corrente, cartão, crédito, câmbio e investimentos) e os pré-requisitos de utilização;
- [O ecossistema do Open Finance Brasil](/docs/ecossistema): Como Receptor e Transmissor se complementam dentro do ecossistema;
- [Jornada de Consentimento para Compartilhamento de Dados](/docs/jornadaconsentimento): Como o usuário autoriza o compartilhamento e as diferenças em relação à jornada de pagamento (prazo do consentimento, escopos, renovação).
...
- [Recepção de Dados](/docs/recepcaodedados): Utilização dos endpoints e instruções de chamada da API do produto.

#### 4. Fluxos técnicos e redirecionamento

- [Funcionamento geral da Iniciação de Pagamentos e Recepção de Dados](/docs/funcionamento): Fluxos de consentimento de dados seguem a mesma lógica geral do ITP (listagem de participantes → criação → redirecionamento → consulta), com diferenças nos escopos e na vigência;
- [Redirecionamento App-to-App e Web](/docs/redirecionamento): Como o usuário deve ser redirecionado à Transmissora de Dados para autorizar o consentimento e como deve retornar ao ambiente da Receptora de Dados.
...
Se você tiver dúvidas ou precisar de assistência técnica, **não hesite em entrar em contato com o *Delivery Manager* da Opus Software designado à sua instituição**. nosso objetivo é garantir que sua experiência com a **Plataforma Opus Open Finance** seja tranquila e bem-sucedida.
